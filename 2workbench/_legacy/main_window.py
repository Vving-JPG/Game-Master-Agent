...
from .widgets.editor_stack import EditorStack
from .widgets.console_tabs import ConsoleTabs
from .widgets.agent_status import AgentStatusPanel
from .widgets.resource_monitor import ResourceMonitorPanel
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        print(f"[HTTP] 控制服务器已启动: http://127.0.0.1:18080")
        print(f"[HTTP] API 文档: curl http://127.0.0.1:18080/health")
    ...
    def _setup_menubar(self):
        ...
        # 首选项
        act_preferences = QAction("首选项...", self)
        act_preferences.setShortcut("Ctrl+,")
        act_preferences.triggered.connect(self._on_open_settings)
        settings_menu.addAction(act_preferences)

        settings_menu.addSeparator()

        # API 设置
        act_api_settings = QAction("API 设置...", self)
        act_api_settings.triggered.connect(self._on_api_settings)
        settings_menu.addAction(act_api_settings)

        # 显示设置
        act_display_settings = QAction("显示设置...", self)
        act_display_settings.triggered.connect(self._on_display_settings)
        settings_menu.addAction(act_display_settings)
    ...
    def _on_open_settings(self):
        ...
        QMessageBox.information(self, "设置", "设置功能即将推出，敬请期待！")
        self.statusbar.showMessage("设置面板开发中")

    def _on_api_settings(self):
        """打开 API 设置"""
        from PyQt6.QtWidgets import QMessageBox
        QMessageBox.information(self, "API 设置", "API 设置功能即将推出，敬请期待！")
        self.statusbar.showMessage("API 设置开发中")

    def _on_display_settings(self):
        """打开显示设置"""
        from PyQt6.QtWidgets import QMessageBox
        QMessageBox.information(self, "显示设置", "显示设置功能即将推出，敬请期待！")
        self.statusbar.showMessage("显示设置开发中")
