...
class AIAssistantService:
    ...
    def __init__(self):
        ...
        self._pending_tasks: set[asyncio.Task] = set()  # 待处理的异步任务
    ...
    def _on_user_message(self, event: Event):
        ...
        task.add_done_callback(self._remove_pending_task)
    ...
    def _handle_task_result(self, task: asyncio.Task) -> None:
        ...
        try:
            ...
            logger.info("用户消息处理任务被取消")
        except Exception as e:
            logger.error(f"用户消息处理任务异常: {e}")
            self._emit_error(f"处理异常: {e}")
            self._set_state(ServiceState.IDLE)

    def _remove_pending_task(self, task: asyncio.Task) -> None:
        """从待处理任务集合中移除已完成任务"""
