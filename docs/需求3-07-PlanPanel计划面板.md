...
> 目标执行者：Trae AI
> 依赖：无
> 产出：`presentation/editor/plan_panel.py`

---

## 1. 设计说明

PlanPanel 显示 AI 生成的执行计划，让用户可以：

1. **查看计划**：以步骤列表形式展示每个步骤的工具、描述和参数
2. **确认执行**：逐步确认或一键确认全部
3. **修改计划**：发送修改意见让 AI 重新规划
4. **取消计划**：放弃当前计划
...
│     工具: read_project                      │
│                                            │
│  ⏳ 步骤 2: 创建提示词 dungeon_exploration  │
│     工具: create_prompt                     │
│     参数: name=dungeon_exploration          │
│           category=scene                    │
│     [展开详情 ▼]                            │
│                                            │
│  ⏳ 步骤 3: 创建技能 trap_detection         │
│     工具: create_skill                      │
│                                            │
│  ─────────────────────────────────────────  │
│  [✅ 确认全部]  [✏️ 修改计划]  [❌ 取消]      │
...
**文件**：`presentation/editor/plan_panel.py`
...
```python
from presentation.editor.plan_panel import PlanPanel, StepCard
...
plan_data = {
    ...
    "steps": [
        ...
        },
        {
            ...
            "description": "创建地下城探索提示词",
            "parameters": {
                "name": "dungeon_exploration",
                "category": "scene",
                "content": "你是一个经验丰富的地下城探索者...",
            },
            "status": "pending",
        },
    ],
...
panel = PlanPanel(plan_data)
assert len(panel._step_cards) == 2

# 更新状态
panel.update_step_status(1, "completed")
assert panel._step_cards[0]._step_data["status"] == "completed"
```
